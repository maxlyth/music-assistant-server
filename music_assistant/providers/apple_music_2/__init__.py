"""Apple Music Library musicprovider support for MusicAssistant."""
...
class AppleMusicProvider(MusicProvider):
    ...
    async def search(
        self, search_query: str, media_types=list[MediaItemType] | None, limit: int = 5
    ) -> SearchResults:
        ...
        if MediaItemType.PLAYLIST in media_types:
    ...
    async def get_library_artists(self) -> AsyncGenerator[Artist, None]:
        """Retrieve library artists from provider."""
    ...
    async def get_library_tracks(self) -> AsyncGenerator[Track, None]:
        ...
        song_catalog_ids = []
        song_library_ids = ["i.kGOgYQziR35z"]
        all_items = await self._get_all_items(endpoint, include="catalog,artists")
        self.logger.warning("Adding %i tracks from library", len(all_items))

        for item in all_items:
            catalog_id = item.get("attributes", {}).get("playParams", {}).get("catalogId")
            if not catalog_id:
                song_library_ids.append(item["id"])
            else:
                song_catalog_ids.append(catalog_id)
        # Obtain catalog info per 200 songs, the documented limit of 300 results in a 504 timeout
        max_limit = 200
        for i in range(0, len(song_library_ids), max_limit):
            library_ids = song_library_ids[i : i + max_limit]
            library_endpoint = "me/library/songs"
            response = await self._get_data(
                library_endpoint, ids=",".join(library_ids), include="catalog,artists"
            )
            for item in response["data"]:
                library_track = self._parse_track(item)
                if library_track:
                    yield library_track
        for i in range(0, len(song_catalog_ids), max_limit):
            ...
            response = await self._get_data(
                catalog_endpoint, ids=",".join(catalog_ids), include="artists"
            )
            for item in response["data"]:
                catalog_track = self._parse_track(item)
                if catalog_track:
                    yield catalog_track
    ...
    async def get_artist(self, prov_artist_id) -> Artist:
        ...
        if self.is_library_id(prov_artist_id):
            endpoint = f"me/library/artists/{prov_artist_id}"
            endpoint_include = ""
        else:
            endpoint = f"catalog/{self._storefront}/artists/{prov_artist_id}"
            endpoint_include = "catalog"
        response = await self._get_data(endpoint, include=endpoint_include, extend="editorialNotes")
        return self._parse_artist(response["data"][0])

    async def get_album(self, prov_album_id) -> Album:
        """Get full album details by id."""
        if self.is_library_id(prov_album_id):
            endpoint = f"me/library/albums/{prov_album_id}"
            endpoint_include = "artists"
        else:
            endpoint = f"catalog/{self._storefront}/albums/{prov_album_id}"
            endpoint_include = "catalog,artists"
        response = await self._get_data(endpoint, include=endpoint_include)
        return self._parse_album(response["data"][0])

    async def get_track(self, prov_track_id) -> Track:
        """Get full track details by id."""
        if self.is_library_id(prov_track_id):
    ...
    async def get_playlist_tracks(self, prov_playlist_id, page: int = 0) -> list[Track]:
        ...
        response = await self._get_data(
            endpoint, include=endpoint_include, limit=page_size, offset=offset
        )
    ...
    async def get_artist_albums(self, prov_artist_id) -> list[Album]:
        ...
        if self.is_library_id(prov_artist_id):
            endpoint = f"me/library/artists/{prov_artist_id}/albums"
        else:
    ...
    async def get_artist_toptracks(self, prov_artist_id) -> list[Track]:
        ...
        if self.is_library_id(prov_artist_id):
            endpoint = f"me/library/artists/{prov_artist_id}/view/top-songs"
        else:
    ...
    async def library_remove(self, prov_item_id, media_type: MediaItemType):
    ...
    async def get_stream_details(self, item_id: str, media_type: MediaItemType) -> StreamDetails:
        ...
        if self.is_library_id(item_id):
            stream_details = StreamDetails(
                ...
                path=stream_metadata["assets"][0]["URL"],
    ...
    def _parse_artist(self, artist_obj):
        ...
        if (
            ...
            return ItemMapping(
                media_type=MediaItemType.ARTIST,
                provider=self.lookup_key,
                item_id=artist_id,
                name=artist_id,
            )
    ...
    def _parse_album(self, album_obj: dict) -> Album | ItemMapping | None:
        ...
        if (
            ...
            # No more details available other than the id, return an ItemMapping
            return ItemMapping(
                media_type=MediaItemType.ALBUM,
                provider=self.lookup_key,
                item_id=album_id,
                name=album_id,
            )
    ...
    def _parse_track(
        ...
        elif artist_name := (attributes.get("artistName") or "Unknown Artist"):
            track.artists = [
                ItemMapping(
                    media_type=MediaItemType.ARTIST,
                    item_id=artist_name,
